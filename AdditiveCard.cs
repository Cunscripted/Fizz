    ...
    IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    [Tooltip("Optional - applies a per-rarity tint/shine (RarityShine shader) whenever this card's instance " +
             "is assigned. Wire this up on prefabs that should show it (e.g. ShopMenuController's " +
             "offerCardPrefab) and leave it unset on plain belt/bottle card prefabs to keep the effect " +
             "scoped to wherever you actually want it - e.g. the shop.")]
    public RarityShineController rarityShine;
    ...
    /// <summary>Sets the artwork Image's sprite to the current additive's icon, and applies rarity shine, if both are set.</summary>
    private void ApplyIcon()
    {
        if (data == null) return;

        // Independent of the icon logic below, so a missing/misconfigured artworkImage
        // doesn't also silently skip the rarity shine.
        rarityShine?.Apply(data.rarity);
